...
module.exports = (options) => {
  return (token, tokenTypeHint) => {
    ...
    logger.debug('token-introspector - authorization value used :' + getSafeSensitiveValue(options.authorization_value));
    logger.debug('token-introspector - trying to introspect the following token :' + getSafeSensitiveValue(data.token));
